...
// Looks up a real Wikipedia summary for the landmark and returns the
// first two sentences of Wikipedia's OWN text, verbatim — no AI
// rewriting, no invented details. Returns null (never a guess) if no
// matching article is found, or if the source is too thin/ambiguous.
...
// abbreviations (U.S., St., Mt., Dr., etc.) are protected first so
// their periods aren't mistaken for sentence endings.
function firstTwoSentences(text) {
  var ABBR = /\b(U\.S|U\.K|St|Dr|Mr|Mrs|Ms|Jr|Sr|vs|etc|approx|No|Ave|Blvd|Mt|Ft|Rd|Co|Inc|Ltd)\./gi;
  var masked = text.replace(ABBR, function(m) { return m.replace(/\./g, "\u0000"); });

  var sentences = masked.match(/[^.!?]+[.!?]+(?:\s|$)/g);
  var result = (!sentences || !sentences.length) ? masked.trim() : sentences.slice(0, 2).join("").trim();
