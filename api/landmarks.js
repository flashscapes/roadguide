export default async function handler(req, res) {
  ...
  const CATEGORY_TYPE_MAP = {
    ...
    nature: ["park", "national_park", "state_park", "botanical_garden", "garden", "hiking_area", "scenic_spot", "beach", "lake", "river", "mountain_peak", "nature_preserve", "observation_deck", "marina", "wildlife_refuge", "wildlife_park"],
    winery: ["winery", "vineyard"],
    park: ["park", "national_park", "state_park"],
    museum: ["museum", "art_museum", "history_museum", "planetarium", "art_gallery"],
    trail: ["hiking_area", "park"],
    family: ["zoo", "aquarium", "amusement_park", "water_park"],
    artsy: ["art_gallery", "art_studio", "cultural_center"],
    music: ["live_music_venue", "concert_hall", "philharmonic_hall", "opera_house"],
    comedy: ["comedy_club"]
  };
  ...
  const EXCLUDED_TYPES = new Set([
    ...
    "sculpture", "monument",
    // Hard-excluded for the same reason as sculpture/monument above:
    // Google occasionally mis-tags a high-traffic location (a busy car
    // dealership can pick up a stray "tourist_attraction" tag in
    // Google's own data) — merely leaving these out of includedTypes
    // isn't enough, since a mis-tagged place can still carry one of
    // the searched-for types too. Applies everywhere, not just Cool
    // Stuff/Dope Detours — a mis-tagged dealership would be equally
    // wrong in any category.
    "car_dealer", "car_repair", "car_rental", "car_wash"
  ]);
  // Artsy is the one category where sculpture/monument are often the
  // actual point of the discovery (a real sculpture park or public art
  // installation), not incidental clutter — so this exclusion applies
  // everywhere EXCEPT Artsy specifically.
  const EXCLUDED_TYPES_FOR_REQUEST = category === "artsy"
    ? new Set([...EXCLUDED_TYPES].filter((t) => t !== "sculpture" && t !== "monument"))
    : EXCLUDED_TYPES;
  ...
  try {
    ...
    let scored = merged
      ...
      .filter((place) => {
        ...
        return !types.some((t) => EXCLUDED_TYPES_FOR_REQUEST.has(t));
      })
      .filter((place) => {
        // Type-based exclusion only catches what Google itself tags
        // as "monument"/"sculpture" — but plenty of famous monuments
        // (Nelson's Column, war memorials, obelisks) are tagged
        // plainly as tourist_attraction with no monument/sculpture
        // type at all. Checking the name directly closes that gap.
        // Skipped for Artsy for the same reason as the type-based
        // exclusion above — a named sculpture/monument is often
        // exactly the discovery Artsy is looking for.
        if (category === "artsy") return true;
        const name = (place.displayName?.text || "").toLowerCase();
        return !/\b(monument|statue|column|obelisk)\b/.test(name);
      })
      .filter((place) => {
        ...
        // Museums/Quirky/Scenic/Artsy deliberately use a much lower
        // review-count bar than the rest of the app — a small,
        // distinctive museum or roadside oddity with only 12 reviews
        // is exactly the kind of discovery these categories exist
        // for, and a hard 50-review requirement would systematically
        // exclude it in favor of more mainstream, higher-traffic
        // places. Rating bar is nudged up slightly in exchange, since
        // review count is no longer doing much filtering work here.
        const RELAXED_FLOOR_CATEGORIES = new Set(["museum", "coolstuff", "nature", "artsy"]);
        if (RELAXED_FLOOR_CATEGORIES.has(category)) {
    ...
    // Wikipedia coverage is now a SOFT signal for Quirky, not a hard
    // gate — funFacts stays on each item as enrichment when available
    // (useful for display, and for the frontend's "isFun"-style
    // bonus), but a genuinely distinctive, lesser-known discovery
    // that simply hasn't been documented on Wikipedia should still be
    // eligible. A hard requirement here would systematically exclude
    // exactly the kind of hidden gem this category exists to surface.
...
// Strips a string down to its meaningful comparable words — used to
// verify a Wikipedia search result is actually ABOUT the place that
// was searched for, not just Wikipedia's closest fuzzy guess when no
// real article exists (e.g. a car dealership or small local business
// with no Wikipedia page at all). Wikipedia's search API always
// returns its best-effort match rather than "no result", so without
// this check, a business with no real article can end up displaying
// a completely unrelated article's text as if it were a fact about
// that specific place.
...
function titleMatchesQuery(query, title) {
  ...
  // Otherwise require meaningful word overlap. When either name is
  // short (2 or fewer significant words — e.g. "Oxbow (river)"), a
  // single shared generic word isn't enough evidence on its own, so
  // full overlap is required; longer names only need at least half
  // their significant words to match.
  const qWords = new Set(q.split(" ").filter((w) => w.length > 2));
  const tWords = new Set(t.split(" ").filter((w) => w.length > 2));
  if (!qWords.size || !tWords.size) return false;
  let overlap = 0;
  qWords.forEach((w) => { if (tWords.has(w)) overlap++; });
  const smaller = Math.min(qWords.size, tWords.size);
  const requiredRatio = smaller <= 2 ? 1 : 0.5;
  return overlap / smaller >= requiredRatio;
}
...
// if no matching English article is found, the source is too
// thin/ambiguous, or the closest match Wikipedia's search returns
// isn't actually about the place that was searched for.
async function getWikipediaFunFacts(name) {
  try {
    ...
    const searchRes = await fetch(searchUrl, { headers: { "User-Agent": "RoadTip/1.0 (https://github.com/flashscapes/roadguide)" } });
    if (!searchRes.ok) return null;
    const searchData = await searchRes.json();
    const title = searchData?.query?.search?.[0]?.title;
    if (!title) return null;
    if (!titleMatchesQuery(name, title)) return null;

    const summaryUrl = "https://en.wikipedia.org/api/rest_v1/page/summary/" + encodeURIComponent(title);
    const summaryRes = await fetch(summaryUrl, { headers: { "User-Agent": "RoadTip/1.0 (https://github.com/flashscapes/roadguide)" } });
