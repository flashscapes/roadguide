export default async function handler(req, res) {
  ...
  // Nature, Winery, Park, Museums, Trails, Screen/Culture) is
  ...
  //
  // Screen/Culture is deliberately NOT "movie_theater" — that would
  ...
  const CATEGORY_TYPE_MAP = {
    culture: ["art_gallery", "art_studio", "cultural_landmark", "plaza", "historical_landmark", "historical_place", "museum", "history_museum", "art_museum", "performing_arts_theater", "television_studio", "tourist_attraction", "event_venue", "cultural_center", "market", "farmers_market", "tour_agency", "visitor_center"],
  ...
  //
  // Culture is the one deliberate exception: genuine sculpture
  // installations are an explicitly requested feature there (Arts &
  // Visual Culture), so that exclusion doesn't apply for this
  // category specifically. Monument stays excluded everywhere,
  // including Culture — it was never actually requested.
  // Statues/monuments/sculptures are hard-excluded everywhere,
  // including Culture — a genuinely famous statue is frequently ALSO
  ...
  // what other types a place also carries. (Culture briefly allowed
  ...
  try {
    ...
    if (singleCategoryTypes) {
      // Single-category search (Historic, Nature, Winery, Park,
      // Museums, Trails, Screen/Culture) — no bucket diversity needed
      // since every result already belongs to the same category;
      // just rank by quality/reviews/proximity/fun and take the best.
