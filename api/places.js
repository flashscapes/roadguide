  ...
  try {
    ...
    const scoredCandidates = merged
      ...
      .map((place) => {
        ...
        if (localVisiting === 'visiting' && (types.includes('coffee_roastery') || types.includes('tea_house'))) localVisitingAdj += 4;
        if (localVisiting === 'local' && types.includes('coffee_shop')) localVisitingAdj += 4;

        // Same modest within-tier proximity tiebreaker as restaurants.js.
        const proximityTiebreak = Math.max(0, 3 - dist * 0.5);
        const tierScore = qualityScore + proximityTiebreak + localVisitingAdj;
