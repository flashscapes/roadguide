  ...
  try {
    ...
    const scoredCandidates = merged
      ...
      .map((place) => {
        ...
        const qualityScore = rating * 10 + Math.log10(effectiveReviewCount + 1) * reviewWeight;

        // localVisiting for coffee: same honest, smaller-effect
        // approach as restaurants.js — coffee_roastery/tea_house read
        // as more distinctive/visit-worthy, plain coffee_shop as more
        // everyday/local, though this is an approximate signal.
        const types = Array.isArray(place.types) ? place.types : [];

        // Local/Visiting: previously only checked coffee-specific
        // types, meaning this had ZERO effect for Bar/Food — fixed to
        // check bar-appropriate types too. wine_bar/cocktail_bar read
        // as more destination-worthy; pub/sports_bar/irish_pub read
        // as more everyday-neighborhood.
        let localVisitingAdj = 0;
        if (localVisiting === 'visiting' && (types.includes('coffee_roastery') || types.includes('tea_house') || types.includes('wine_bar') || types.includes('cocktail_bar'))) localVisitingAdj += 4;
        if (localVisiting === 'local' && (types.includes('coffee_shop') || types.includes('pub') || types.includes('sports_bar') || types.includes('irish_pub'))) localVisitingAdj += 4;

        // funInformative: previously accepted as a parameter but
        // never actually used anywhere — had ZERO effect. brewery/
        // brewpub read as more "informative" (craft/process-oriented,
        // teaches you how something is made); sports_bar/cocktail_bar
        // read as more purely "fun"/social. Coffee has a weaker
        // signal here (coffee_roastery is the one clear match).
        let funInformativeAdj = 0;
        if (funInformative === 'informative' && (types.includes('brewery') || types.includes('brewpub') || types.includes('coffee_roastery'))) funInformativeAdj += 4;
        if (funInformative === 'fun' && (types.includes('sports_bar') || types.includes('cocktail_bar'))) funInformativeAdj += 4;
