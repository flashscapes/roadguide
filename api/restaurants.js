export default async function handler(req, res) {
  ...
  const { lat, lng, radius = 5000, hidden } = req.query;
  ...
  try {
    ...
    const restaurants = places
      ...
      .map((place) => {
        ...
        // review counts to completely overwhelm rating. In "hidden"
        // mode (used by Hook Me Up's Hidden Restaurant), review
        // volume is dropped from scoring almost entirely — the whole
        // point is to let a genuinely good, under-the-radar place
        // compete on rating alone, not lose to popularity.
        const reviewScore = Math.log10(reviewCount + 1);

        const score = hiddenMode
          ? rating * 10 + reviewScore * 0.2
          : rating * 10 + reviewScore * 2;
