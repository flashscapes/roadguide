export default async function handler(req, res) {
  ...
  try {
    ...
    //
    // Tries increasingly wide radii (3km, then 10km, 25km, 50km — the
    // max this API allows) and stops as soon as there's enough named
    // context to work with. Sparse rural spots need a wider net than
    // a dense downtown block does.
    ...
    for (const radius of radiiToTry) {
      places = await fetchNearby(radius);
      radiusUsed = radius;
      if (places.length >= 3) break; // enough named context — stop widening
    }
    ...
    return res.status(200).json({
      story: story ? story.trim() : "I couldn't come up with anything for this area right now.",
