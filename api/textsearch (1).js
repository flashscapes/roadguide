export default async function handler(req, res) {
  ...
  // HONEST DESIGN NOTE: this endpoint now requires a real website AND
  // real posted weekly hours AND a currently-active status before a
  // result qualifies at all — no vague "maybe" listings. This means
  // one-time annual festivals will mostly disappear here (they don't
  // fit into Google's weekly-hours schema at all), while genuine
  // weekly recurring things (a Thursday food truck night, a Saturday
  // farmers market) can work well IF the organizer actually
  // maintains their Google listing's hours — coverage still depends
  // entirely on that, which we have no way to guarantee or predict.
  ...
  try {
    ...
    const finalPlaces = merged
      ...
      .filter((place) => {
        // STRICT bar, deliberately much tighter than the rest of this
        // app: a real photo, a real website, real posted hours data,
        // AND currently marked open based on that schedule. This is
        // what actually distinguishes "a genuine weekly recurring
        // thing with a maintained Google listing" from "a stale or
        // annual-only entry we can't verify is happening at all" —
        // if any of these are missing, we simply don't know enough to
