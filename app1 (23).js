...
function scoreCuratedForExplore(lm) {
  ...
  var proximityBonus = distanceMiles != null ? Math.max(0, 3 - distanceMiles * 0.2) : 0;
  var funBonus = curatedIsFun(lm) ? 3 : 0;
...
function renderWorldLandmarks() {
  ...
  var liveScored = worldLandmarkResults.map(function(lm) {
    ...
    var proximityBonus = dist != null ? Math.max(0, 3 - dist * 0.2) : 0;
    var funBonus = lm.isFun ? 3 : 0;
...
function exploreBlendedCardHTML(item, index, isHero) {
  ...
  if (isCurated) {
    ...
    var photoUrl = item.photoRef
      ...
      : '';
    thumbHTML = photoUrl
      ? '<img class="rest-thumb" src="' + photoUrl + '" alt="' + escHtml(item.name) + '" '
        + 'onerror="this.outerHTML=\'<div class=&quot;rest-thumb-emoji&quot;>\uD83D\uDCCD</div>\'">'
      : '<div class="rest-thumb-emoji">📍</div>';
  }
  ...
  return '<div class="' + cardClass + '" onclick="openExploreBlendedItem(' + index + ')">'
       +   '<div class="rest-top">'
       +     thumbHTML
       +     '<div class="card-meta">'
