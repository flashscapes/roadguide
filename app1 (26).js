...
function setCat(cat, el) {
  ...
  if (cat === 'restaurants' || cat === 'coffee' || cat === 'bar' || cat === 'film') {
    ...
    else if (cat === 'film') loadScreenCulture();
...
function getFiltered() {
  ...
  if (activeCategory === 'restaurants' || activeCategory === 'coffee' || activeCategory === 'bar' || activeCategory === 'film') return [];
...
function onPos(pos) {
  ...
  if (activeCategory === 'film') {
    if (!screenCultureLoaded && !screenCultureLoading) loadScreenCulture();
    return;
  }
  if (!activeCategory) {
...
// ─────────────────────────────────────────────────────────
var screenCultureResults = [];
var screenCultureLoaded = false;
var screenCultureLoading = false;
var screenCultureBlendedResults = [];
var SCREEN_CULTURE_MAX_MILES = 150;

function isCuratedFilmEntry(lm) {
  return /movie|tv|film/.test((lm.cat || '').toLowerCase());
}

function loadScreenCulture() {
  if (screenCultureLoading) return;
  if (userLat === null || userLon === null) {
    setMsg('Waiting for your GPS location…');
    var waitTimer = setInterval(function() {
      if (userLat !== null && userLon !== null) {
  ...
  screenCultureLoading = true;
  screenCultureLoaded  = false;
  setMsg('Searching for nearby Screen & Culture spots…');
  var url = 'https://roadguide-lime.vercel.app/api/landmarks?category=film&lat=' + encodeURIComponent(userLat)
          + '&lng=' + encodeURIComponent(userLon) + '&radius=20000';
  fetch(url)
    .then(function(response) {
      if (!response.ok) throw new Error('HTTP ' + response.status);
    ...
    .then(function(data) {
      screenCultureLoading = false;
      screenCultureLoaded  = true;
      screenCultureResults = Array.isArray(data.landmarks) ? data.landmarks : [];
      renderScreenCulture();
    })
    .catch(function(error) {
      screenCultureLoading = false;
      screenCultureLoaded  = true;
      console.error('Screen & Culture loading error:', error);
      screenCultureResults = [];
      setMsg('Live search unavailable right now — showing your curated spots.');
      renderScreenCulture();
    });
}

function renderScreenCulture() {
  ...
  var curatedFilm = ALL_LANDMARKS.filter(isCuratedFilmEntry).map(function(lm) {
    var dist = (userLat !== null && userLon !== null && lm.lat != null && lm.lon != null)
      ? haversine(userLat, userLon, lm.lat, lm.lon) : null;
    if (dist != null && dist > SCREEN_CULTURE_MAX_MILES) return null;
    var dir = (userLat !== null && userLon !== null && lm.lat != null && lm.lon != null)
      ? bearing(userLat, userLon, lm.lat, lm.lon) : '';
    return {
      origin: 'curated', name: lm.name, county: lm.county || '', emoji: lm.emoji || '🎬',
  ...
  var liveFilm = screenCultureResults.map(function(lm) {
    var lat = (lm.latitude  != null) ? Number(lm.latitude)  : NaN;
    var lon = (lm.longitude != null) ? Number(lm.longitude) : NaN;
    var dist = (userLat !== null && Number.isFinite(lat) && Number.isFinite(lon)) ? haversine(userLat, userLon, lat, lon) : null;
    var dir  = (userLat !== null && Number.isFinite(lat) && Number.isFinite(lon)) ? bearing(userLat, userLon, lat, lon) : '';
  ...
  var combined = curatedFilm.concat(liveFilm);
  combined.sort(function(a, b) { return (a.dist == null ? Infinity : a.dist) - (b.dist == null ? Infinity : b.dist); });
  combined = combined.slice(0, 28);

  screenCultureBlendedResults = combined;

  if (!combined.length) {
    setMsg('No nearby Screen & Culture spots found.');
    if (countEl) countEl.textContent = 'No Screen & Culture spots found nearby';
    if (dc) dc.textContent = '0';
    if (cards) cards.innerHTML = '<div class="card"><div class="card-placeholder">No nearby Screen & Culture spots found</div></div>';
    if (list) list.innerHTML = '';
    return;
  }
  ...
  renderScreenCultureCards();
  renderScreenCultureList();
...
function renderScreenCultureCards() {
  var cards = document.getElementById('cards');
  if (!cards) return;
  var html = '';
  for (var i = 0; i < HERO_COUNT; i++) {
    var item = screenCultureBlendedResults[i];
    html += item
      ? exploreBlendedCardHTML(item, i, true, 'openScreenCultureItem')
      : '<div class="card"><div class="card-placeholder">No additional spots</div></div>';
  }
  cards.innerHTML = html;
}

function renderScreenCultureList() {
  var list = document.getElementById('list');
  if (!list) return;
  var items = screenCultureBlendedResults.slice(HERO_COUNT, HERO_COUNT + 21);
  if (!items.length) {
  ...
  var html = '';
  for (var i = 0; i < items.length; i++) {
    html += exploreBlendedCardHTML(items[i], i + HERO_COUNT, false, 'openScreenCultureItem');
  }
  list.innerHTML = html;
}

function openScreenCultureItem(index) {
  var item = screenCultureBlendedResults[index];
  if (!item) return;
  if (item.origin === 'curated') {
    if (item.lat != null && item.lon != null) {
      window.open('https://www.google.com/maps/dir/?api=1&destination=' + encodeURIComponent(item.lat + ',' + item.lon), '_blank');
