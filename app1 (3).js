...
(function initStoryButton() {
  ...
  // Browsers default to their most basic voice unless a better one is
  // explicitly picked. This looks for a known higher-quality voice
  // (and prefers the "Enhanced"/"Premium" version of it if the device
  // has one installed), falling back gracefully if not.
  function pickBestVoice() {
    if (cachedVoice) return cachedVoice;
    var voices = window.speechSynthesis.getVoices() || [];
    if (!voices.length) return null;

    function isEnglish(v) { return v.lang && v.lang.indexOf('en') === 0; }
    function isPremium(v) {
      var id = (v.voiceURI || '') + ' ' + (v.name || '');
      return /enhanced|premium|natural/i.test(id);
    }

    var englishVoices = voices.filter(isEnglish);
    if (!englishVoices.length) englishVoices = voices;

    var preferredNames = ['Samantha', 'Ava', 'Serena', 'Nicky', 'Google US English'];

    for (var i = 0; i < preferredNames.length; i++) {
      var premiumMatch = englishVoices.find(function(v) {
        return v.name.indexOf(preferredNames[i]) !== -1 && isPremium(v);
      });
      if (premiumMatch) { cachedVoice = premiumMatch; return premiumMatch; }
    }
    for (var j = 0; j < preferredNames.length; j++) {
      var anyMatch = englishVoices.find(function(v) { return v.name.indexOf(preferredNames[j]) !== -1; });
      if (anyMatch) { cachedVoice = anyMatch; return anyMatch; }
    }
    var premiumEnglish = englishVoices.find(isPremium);
    if (premiumEnglish) { cachedVoice = premiumEnglish; return premiumEnglish; }

    var localEnglish = englishVoices.find(function(v) { return v.localService; });
    cachedVoice = localEnglish || englishVoices[0] || voices[0] || null;
    return cachedVoice;
  }

  if ('speechSynthesis' in window) {
    // Voice lists load asynchronously on some browsers (notably Safari) —
    // this warms the cache as soon as the real list is ready.
    window.speechSynthesis.onvoiceschanged = function() { pickBestVoice(); };
    pickBestVoice();
  }
  ...
  function speak(text, onDone) {
    ...
    utter.rate = 0.97;
    utter.pitch = 1.0;
    utter.onend = function() { onDone && onDone(); };
    utter.onerror = function() { onDone && onDone(); };
    window.speechSynthesis.speak(utter);
  }

  window.tellMeTheStory = function() {
    if (busy) return; // ignore extra taps while one request is already running
    if (userLat === null || userLon === null) {
      speak("I don't have a location to work with yet.");
      return;
    }
    ...
    speak("What's the story here?", function() {
      fetch('https://roadguide-lime.vercel.app/api/story?lat=' + encodeURIComponent(userLat)
        ...
        .then(function(result) {
          ...
          var text;
          if (!result.ok) {
            // The request went through fine, but the server itself reported
            // a problem (e.g. a missing API key). Speak the real reason
            // instead of a generic "nothing here" line, so setup issues
            // are obvious immediately instead of looking like the AI
        ...
        .catch(function(err) {
          ...
          setState('speaking');
          speak("Something went wrong reaching the story feature. Check your connection and try again.", function() {
            busy = false;
            btn.disabled = false;
            setState(null);
          });
