...
// "Explore" (formerly "All") shows curated county stories when
// you're near them, live worldwide hotspots otherwise. This distance
// (miles) is how close you need to be to your NEAREST real curated
// landmark to count as "near your counties" — computed from the
// actual dataset rather than a hardcoded state boundary, so it
// automatically matches whatever you've actually curated.
var EXPLORE_CURATED_RADIUS_MILES = 50;
var exploreLiveMode = false;

function nearestCuratedLandmarkMiles() {
  if (userLat === null || userLon === null || !ALL_LANDMARKS.length) return null;
  var min = Infinity;
  for (var i = 0; i < ALL_LANDMARKS.length; i++) {
    var lm = ALL_LANDMARKS[i];
    if (lm.lat == null || lm.lon == null) continue;
    var d = haversine(userLat, userLon, lm.lat, lm.lon);
    if (d < min) min = d;
  }
...
function getFiltered() {
  ...
  if (activeCategory === 'restaurants' || activeCategory === 'coffee' || activeCategory === 'bar') return [];
...
function sortAndRender() {
  ...
  if (!activeCategory) {
    // "Explore" (formerly "All"): show curated county stories when
    // you're actually near them; otherwise fall back to a live,
    // worldwide, review-filtered hotspot search automatically — no
    // separate tab needed. exploreLiveMode tracks which mode is
    // active so late-arriving async renders don't land in the wrong
    // place if you've since switched tabs.
    var nearestMiles = nearestCuratedLandmarkMiles();
    exploreLiveMode = (nearestMiles !== null && nearestMiles > EXPLORE_CURATED_RADIUS_MILES);
    if (exploreLiveMode) {
    ...
    // Otherwise fall through to the normal curated rendering below.
  }
...
function renderPlacesHeroCards(cat) {
  ...
  for (var i = 0; i < HERO_COUNT; i++) {
    ...
    var photoUrl = p.photoRef
      ...
      : '';
    var thumbHTML = photoUrl
      ? '<img class="rest-thumb" src="' + photoUrl + '" alt="' + escHtml(p.name) + '" '
...
function renderWorldLandmarks() {
  if (activeCategory !== null || !exploreLiveMode) return;
  var cards   = document.getElementById('cards');
  var list    = document.getElementById('list');
  var sep     = document.querySelector('.sep');
  var lbl     = document.getElementById('listLabel');
...
(function initBackgroundMusic() {
  ...
  function attemptAutoStart() {
    ...
    audio.play().catch(function(err) {
      // Autoplay blocked (or file not found yet) — not an error the
      // user needs to see; the toggle button still lets them start
      // it manually with a direct, explicit tap.
  ...
  window.toggleMusicMute = function() {
    if (audio.paused) {
      // Not yet playing (e.g. autoplay attempt failed entirely) — a
      // direct tap on this button is an unambiguous user gesture, so
      // try starting it fresh here too.
      audio.muted = false;
      audio.play().catch(function(err) {
