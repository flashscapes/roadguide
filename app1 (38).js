...
// ─────────────────────────────────────────────────────────
// ADD COOL STUFF CATEGORY BUTTON (worldwide, Places-powered,
// blended with curated entries) — positioned right after Bar/Food,
// per explicit request. It's the app's default/starting category
// (see activeCategory's initial value), so it also carries the
// initial "active" visual state.
// ─────────────────────────────────────────────────────────
(function addCoolStuffChip() {
  var catScroll = document.getElementById('catScroll');
  if (!catScroll) return;
  if (document.getElementById('coolStuffChip')) return;
  var btn = document.createElement('button');
  btn.id = 'coolStuffChip';
  btn.className = 'cat-chip active';
  btn.textContent = '😎 Cool Stuff';
  btn.onclick = function() { setCat('coolstuff', btn); };
  // 4th chip (right after Bar/Food)
  catScroll.insertBefore(btn, catScroll.children[3] || null);
})();
...
function setCat(cat, el) {
  ...
  if (cat === 'restaurants' || cat === 'coffee' || cat === 'bar' || cat === 'coolstuff') {
    ...
    else if (cat === 'coolstuff') loadCoolStuff();
...
function getFiltered() {
  ...
  if (activeCategory === 'restaurants' || activeCategory === 'coffee' || activeCategory === 'bar' || activeCategory === 'coolstuff' || activeCategory === 'hookmeup' || activeCategory === 'dopedetours') return [];
...
function onPos(pos) {
  ...
  if (activeCategory === 'coolstuff') {
    if (!coolStuffLoaded && !coolStuffLoading) loadCoolStuff();
    return;
  }
...
// ─────────────────────────────────────────────────────────
var coolStuffResults = [];
var coolStuffLoaded = false;
var coolStuffLoading = false;
var coolStuffBlendedResults = [];
var COOLSTUFF_MAX_MILES = 20;
...
function loadCoolStuff() {
  if (coolStuffLoading) return;
  if (userLat === null || userLon === null) {
    setMsg('Waiting for your GPS location…');
    var waitTimer = setInterval(function() {
      if (userLat !== null && userLon !== null) {
  ...
  coolStuffLoading = true;
  coolStuffLoaded  = false;
  setMsg('Searching for nearby Cool Stuff spots…');
  var url = 'https://roadguide-lime.vercel.app/api/landmarks?category=coolstuff&lat=' + encodeURIComponent(userLat)
  ...
  fetch(url)
    ...
    .then(function(data) {
      coolStuffLoading = false;
      coolStuffLoaded  = true;
      coolStuffResults = Array.isArray(data.landmarks) ? data.landmarks : [];
      renderCoolStuff();
    })
    .catch(function(error) {
      coolStuffLoading = false;
      coolStuffLoaded  = true;
      console.error('Cool Stuff loading error:', error);
      coolStuffResults = [];
      setMsg('Live search unavailable right now — showing your curated spots.');
      renderCoolStuff();
    });
...
function renderCoolStuff() {
  if (activeCategory !== 'coolstuff') return;
  var cards   = document.getElementById('cards');
  var list    = document.getElementById('list');
  var sep     = document.querySelector('.sep');
  var lbl     = document.getElementById('listLabel');
  ...
  var curatedCoolStuff = ALL_LANDMARKS.filter(isCuratedCoolStuffEntry).map(function(lm) {
    var dist = (userLat !== null && userLon !== null && lm.lat != null && lm.lon != null)
      ? haversine(userLat, userLon, lm.lat, lm.lon) : null;
    if (dist != null && dist > COOLSTUFF_MAX_MILES) return null;
    var dir = (userLat !== null && userLon !== null && lm.lat != null && lm.lon != null)
      ? bearing(userLat, userLon, lm.lat, lm.lon) : '';
    return {
      origin: 'curated', name: lm.name, county: lm.county || '', emoji: lm.emoji || '😎',
  ...
  var liveCoolStuff = coolStuffResults.map(function(lm) {
    var lat = (lm.latitude  != null) ? Number(lm.latitude)  : NaN;
    var lon = (lm.longitude != null) ? Number(lm.longitude) : NaN;
    var dist = (userLat !== null && Number.isFinite(lat) && Number.isFinite(lon)) ? haversine(userLat, userLon, lat, lon) : null;
    var dir  = (userLat !== null && Number.isFinite(lat) && Number.isFinite(lon)) ? bearing(userLat, userLon, lat, lon) : '';
    ...
    return item.dist == null || item.dist <= COOLSTUFF_MAX_MILES;
  });

  var combined = curatedCoolStuff.concat(liveCoolStuff);
  combined.sort(function(a, b) { return (a.dist == null ? Infinity : a.dist) - (b.dist == null ? Infinity : b.dist); });
  combined = combined.slice(0, 28);

  coolStuffBlendedResults = combined;

  if (!combined.length) {
    setMsg('No genuinely interesting Cool Stuff spots found within 15 miles right now.');
    if (countEl) countEl.textContent = 'No Cool Stuff spots found nearby';
    if (dc) dc.textContent = '0';
    if (cards) cards.innerHTML = '<div class="card"><div class="card-placeholder">No nearby Cool Stuff spots found</div></div>';
    if (list) list.innerHTML = '';
    return;
  }
...
function renderCoolStuffCards() {
  var cards = document.getElementById('cards');
  if (!cards) return;
  var html = '';
  for (var i = 0; i < HERO_COUNT; i++) {
    var item = coolStuffBlendedResults[i];
    html += item
      ? exploreBlendedCardHTML(item, i, true, 'openCoolStuffItem')
      : '<div class="card"><div class="card-placeholder">No additional spots</div></div>';
  }
  cards.innerHTML = html;
}

function renderCoolStuffList() {
  var list = document.getElementById('list');
  if (!list) return;
  var items = coolStuffBlendedResults.slice(HERO_COUNT, HERO_COUNT + 21);
  if (!items.length) {
  ...
  var html = '';
  for (var i = 0; i < items.length; i++) {
    html += exploreBlendedCardHTML(items[i], i + HERO_COUNT, false, 'openCoolStuffItem');
  }
  list.innerHTML = html;
}

function openCoolStuffItem(index) {
  var item = coolStuffBlendedResults[index];
  if (!item) return;
  if (item.origin === 'curated') {
    if (item.lat != null && item.lon != null) {
      window.open('https://www.google.com/maps/dir/?api=1&destination=' + encodeURIComponent(item.lat + ',' + item.lon), '_blank');
...
function loadHookMeUpMood(mood) {
  ...
  if (mood === 'eat') {
    hookMeUpFetchRestaurants();
  } else if (mood === 'museums') {
    hookMeUpFetchBlended('museum', isCuratedMuseumEntry, '🔭');
  } else if (mood === 'surprise' || mood === 'weird' || mood === 'seeit') {
    ...
    hookMeUpFetchTextSearch('farmers market', 10,
      '📅 Recurring market — exact days/hours vary. Check current schedule before visiting.', '🥕');
  } else if (mood === 'streetfair') {
    hookMeUpFetchTextSearch('street fair', 10,
      '📅 Recurring or seasonal event — exact dates uncertain. Check current schedule before visiting.', '🎪');
    ...
    hookMeUpFetchTextSearch(
      ...
      10, '🎉 May be a recurring or seasonal event — verify current dates before visiting.', '🎉');
  } else {
...
function hookMeUpFetchBlended(categoryParam, curatedMatcher, fallbackEmoji) {
  ...
  hookMeUpFetchCategory(categoryParam).then(function(liveResults) {
    var live = liveResults.map(function(lm) {
      var lat = (lm.latitude  != null) ? Number(lm.latitude)  : NaN;
      var lon = (lm.longitude != null) ? Number(lm.longitude) : NaN;
    ...
    hookMeUpRenderResults(curated.concat(live));
  }).catch(function(error) {
    console.error('Hook Me Up loading error:', error);
    // Graceful degrade: curated results (if any) don't depend on the
    // failed fetch, so still show those rather than a blank error.
    if (curated.length) {
      hookMeUpRenderResults(curated);
    } else {
...
function hookMeUpFetchLiveOnly(categoryParam) {
  hookMeUpFetchCategory(categoryParam).then(function(liveResults) {
    var live = liveResults.map(function(lm) {
      var lat = (lm.latitude  != null) ? Number(lm.latitude)  : NaN;
      var lon = (lm.longitude != null) ? Number(lm.longitude) : NaN;
...
// Farmers Markets / Street Fairs / Festivals & Fairs — these are
// recurring COMMUNITY EVENTS, not permanent places, and Places API
// has no field anywhere for event dates or recurrence (verified
// directly against Google's current schema). A result here means
// "Google's system has accumulated enough reviews/photos to treat
// this as a persistent entry" — never confirmation it's happening on
// any particular day. The caveat text is baked directly into what's
// displayed, every time, not just mentioned once and forgotten.
function hookMeUpFetchTextSearch(queryString, radiusMiles, caveatText, fallbackEmoji) {
    ...
    .then(function(data) {
      ...
      var live = results.map(function(p) {
        ...
        return Object.assign({}, p, {
          ...
          emoji: fallbackEmoji,
          funFacts: caveatText // the caveat IS the displayed description, always
        });
...
function hookMeUpRenderResults(pool) {
  ...
  var picks = hookMeUpRandomSample(pool, 3, 5);
  ...
  fetch(dirUrl)
    ...
    .then(function(resultsPerWaypoint) {
      ...
      var scored = filtered.map(function(place) {
        ...
        var score = typeWeight + qualityBonus + funBonus;
...
(function initDevTestLocationTool() {
  ...
  select.addEventListener('change', function() {
    ...
    try {
      ...
      } else if (activeCategory === 'coolstuff') {
        loadCoolStuff();
...
(function initBackgroundMusic() {
  ...
  audio.volume = 0.667; // 2/3 volume, set once before any playback attempt
