...
function loadHookMeUpMood(mood) {
  ...
  if (mood === 'museums') {
    ...
    hookMeUpFetchLiveOnly('music', true);
  } else {
    var cat = HOOKMEUP_LIVE_CATEGORY[mood];
    if (!cat) return;
    hookMeUpFetchLiveOnly(cat, false);
  }
}
...
function hookMeUpFetchLiveOnly(categoryParam, requireActiveNow) {
  hookMeUpFetchCategory(categoryParam).then(function(liveResults) {
    var live = liveResults.map(function(lm) {
      var lat = (lm.latitude  != null) ? Number(lm.latitude)  : NaN;
      var lon = (lm.longitude != null) ? Number(lm.longitude) : NaN;
    ...
    if (requireActiveNow) {
      // Same strict bar as Farmers Markets/Street Fairs/Food Trucks:
      // require a real website AND confirmed open-right-now status.
      // Without this, "Music" just meant "any music venue exists
      // nearby" — this narrows it to "verifiably active right now".
      live = live.filter(function(item) { return item.websiteUri && item.openNow === true; });
    }
