...
(function initStoryButton() {
  ...
  window.tellMeTheStory = function() {
    ...
    // Start the location lookup + AI call immediately, in parallel with
    // the spoken prompt below, instead of waiting for the prompt to
    // finish first. This is what caused the long dead-air gap before:
    // the (slower) lookup only started AFTER "What's the story here?"
    // had already finished playing, so the two waits stacked instead
    // of overlapping.
    var storyPromise = fetch('https://roadguide-lime.vercel.app/api/story?lat=' + encodeURIComponent(userLat)
