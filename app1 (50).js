...
// Restore the visual state of the three dropdowns to match, so the
// UI honestly reflects what's already active rather than showing
// blank "Select…" while secretly having a preference applied.
(function restorePreferenceDropdowns() {
  var lv = document.getElementById('prefLocalVisiting');
  var rh = document.getElementById('prefReviewedHidden');
  var fi = document.getElementById('prefFunInformative');
  if (lv && activePreferences.localVisiting) lv.value = activePreferences.localVisiting;
  if (rh && activePreferences.reviewedHidden) rh.value = activePreferences.reviewedHidden;
  if (fi && activePreferences.funInformative) fi.value = activePreferences.funInformative;
})();
...
function onPreferenceChange() {
  ...
  var rh = document.getElementById('prefReviewedHidden');
  var fi = document.getElementById('prefFunInformative');
  activePreferences.localVisiting  = (lv && lv.value) ? lv.value : null;
  activePreferences.reviewedHidden = (rh && rh.value) ? rh.value : null;
  activePreferences.funInformative = (fi && fi.value) ? fi.value : null;
  ...
  // to empty (roadtipStorageSet already handles both cases).
  ...
  // Re-rank/refresh whatever is currently on screen so the effect is
  // immediately visible — never require a full app reload. Only the
  // currently active category needs to react; nothing else is
  // touched, and no unnecessary API calls are made for categories
  // the user isn't even looking at right now.
  ...
  // If the user has now set all three preferences, close the panel
  // automatically — they've clearly finished, no need to make them
  // also tap Done. Setting fewer than three still requires Done.
  if (activePreferences.localVisiting && activePreferences.reviewedHidden && activePreferences.funInformative) {
