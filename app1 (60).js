...
//
// Done deliberately has no data-driven "ready" state at all — it
// never anticipates that the user is finished based on how many
// chips are selected (whether freshly tapped or restored from a
// prior session). It stays visually neutral until the literal moment
// it's pressed, via the #preferencesDoneBtn:active CSS rule.
...
function onPreferenceChipClick(chipEl) {
  ...
  // Deliberately no Done-button state update here. Done should never
  // anticipate that the user is "ready" based on chip selections —
  // it stays visually neutral until the user actually presses it (see
  // the #preferencesDoneBtn:active CSS rule), full stop.
...
function landmarkPreferenceAdjustment(bucket, isFun) {
  ...
  if (p.funInformative === 'cultural' && (bucket === 'historical_cultural' || bucket === 'museum_gallery')) adj += 10;
...
// CATEGORY FILTER
// ─────────────────────────────────────────────────────────
// Thin wrapper: briefly fades/shifts the existing result area out,
// runs the exact original setCat logic (below, renamed setCatInner)
// unchanged, then fades the new results back in. Purely visual —
// no switching logic, data, or timing of loads/renders is altered
// beyond this short cosmetic delay.
function setCat(cat, el) {
  var fadeEls = [
    document.getElementById('cards'),
    document.getElementById('list'),
    document.querySelector('.sep'),
    document.getElementById('listLabel'),
    document.getElementById('eventsPane')
  ].filter(Boolean);
  fadeEls.forEach(function(e) { e.classList.add('cat-fade'); });
  setTimeout(function() {
    setCatInner(cat, el);
    requestAnimationFrame(function() {
      requestAnimationFrame(function() {
        fadeEls.forEach(function(e) { e.classList.remove('cat-fade'); });
      });
    });
  }, 80);
}

function setCatInner(cat, el) {
...
function loadDopeDetours(destination) {
  ...
  fetch(dirUrl)
    ...
    .then(function(resultsPerWaypoint) {
      ...
      //
      // Previously this ceiling was fixed at 3000 regardless of
      // preference, which meant "Highest Reviewed" had ZERO effect on
      // Dope Detours — the whole point of that preference (surfacing
      // well-known places) was blocked outright by this filter. When
      // Highest Reviewed is active, the ceiling lifts so popular stops
      // can actually appear; Hidden Gems (or no preference) keeps the
      // original, tighter ceiling.
      var dopeDetoursHighestReviewed = activePreferences.reviewedHidden === 'highestReviewed';
      var DOPE_DETOURS_REVIEW_CEILING = dopeDetoursHighestReviewed ? Infinity : 3000;
      ...
      // Explore/Cool Stuff. Review count is deliberately NOT rewarded
      // by default (or under Hidden Gems) — rewarding review volume is
      // exactly what would surface major/famous destinations instead
      // of quirky, lesser-known ones. Under Highest Reviewed, though,
      // the whole point is the opposite: a real, tiered reviewBonus is
      // added (same tiers used for Restaurants/Coffee/Bar) so
      // well-reviewed stops actually rank higher along the route.
      var scored = filtered.map(function(place) {
        ...
        var reviewCount = Number(place.reviewCount || 0);
        var reviewBonus = 0;
        if (dopeDetoursHighestReviewed) {
          if (reviewCount >= 1000) reviewBonus = 15;
          else if (reviewCount >= 300) reviewBonus = 10;
          else if (reviewCount >= 100) reviewBonus = 5;
        }
        var score = typeWeight + qualityBonus + funBonus + prefAdj + reviewBonus;
...
// ═════════════════════════════════════════════════════════
// CITY SEARCH — real user-facing feature (replaces the earlier
// developer-only test-location dropdown). Lets anyone look up
// restaurant/landmark results for any city or place they type,
// instead of only their current GPS location. Overrides only the
...
// ═════════════════════════════════════════════════════════
var citySearchLat   = null;  // non-null while a searched city is active
var citySearchLon   = null;
var citySearchLabel = null;
var realGpsLat   = null;  // last known REAL device GPS reading, always kept up to date
var realGpsLon   = null;

(function initCitySearchTool() {

  var box = document.createElement('div');
  box.id = 'citySearchBox';
  box.style.cssText =
    'position:fixed;bottom:calc(10px + env(safe-area-inset-bottom,0px));right:calc(10px + env(safe-area-inset-right,0px));z-index:9999;max-width:62vw;' +
    'background:rgba(8,12,20,.9);border:1px solid rgba(201,168,76,.35);border-radius:12px;' +
    'padding:8px 9px;box-shadow:0 4px 16px rgba(0,0,0,.4);';

  var activeTag = document.createElement('div');
  activeTag.id = 'citySearchActiveTag';
  activeTag.style.cssText =
    'display:none;align-items:center;gap:6px;margin-bottom:7px;' +
    "font-family:'DM Sans',sans-serif;font-size:11.5px;color:var(--glo);";

  var activeTagLabel = document.createElement('span');
  var clearBtn = document.createElement('span');
  clearBtn.textContent = '✕';
  clearBtn.setAttribute('role', 'button');
  clearBtn.setAttribute('aria-label', 'Clear searched city, return to current location');
  clearBtn.style.cssText = 'margin-left:auto;color:var(--dim);font-size:13px;cursor:pointer;padding:0 3px;';

  activeTag.appendChild(activeTagLabel);
  activeTag.appendChild(clearBtn);
  ...
  input.setAttribute('aria-label', 'Search restaurants and landmarks by city');
  input.style.cssText =
    'flex:1;min-width:0;background:transparent;border:none;outline:none;' +
    "color:var(--pale);font-family:'DM Sans',sans-serif;font-size:12.5px;";

  var goBtn = document.createElement('button');
  goBtn.id = 'citySearchBtn';
  goBtn.type = 'button';
  goBtn.textContent = '🔍';
  goBtn.setAttribute('aria-label', 'Search');
  goBtn.style.cssText =
    'flex-shrink:0;width:26px;height:26px;border-radius:8px;border:none;' +
    'background:linear-gradient(135deg,#e8c97a,#c9a84c);color:#1a1408;' +
    'font-size:13px;display:flex;align-items:center;justify-content:center;cursor:pointer;';

  row.appendChild(input);
  row.appendChild(goBtn);
  box.appendChild(activeTag);
  box.appendChild(row);
  document.body.appendChild(box);

  function setLoading(isLoading) {
    goBtn.textContent = isLoading ? '…' : '🔍';
  ...
  function refreshActiveCategory() {
    ...
    try {
      ...
      console.error('[City Search] Error refreshing results:', err);
    }
  }
  ...
  function runCitySearch() {
    ...
    setLoading(true);
    fetch('https://roadguide-lime.vercel.app/api/geocode?query=' + encodeURIComponent(query))
      .then(function(res) { return res.json().then(function(data) { return { ok: res.ok, data: data }; }); })
      .then(function(result) {
        setLoading(false);
        if (!result.ok || result.data.lat == null) {
          input.placeholder = 'City not found — try again';
          input.value = '';
          return;
        }
        citySearchLat = result.data.lat;
        citySearchLon = result.data.lng;
        citySearchLabel = result.data.formattedAddress || query;
        userLat = citySearchLat;
        userLon = citySearchLon;

        activeTagLabel.textContent = '📍 ' + citySearchLabel;
        activeTag.style.display = 'flex';
        input.value = '';
        input.placeholder = 'Search a city…';
        refreshActiveCategory();
      })
      .catch(function(err) {
        console.error('[City Search] Lookup failed:', err);
        setLoading(false);
  ...
  goBtn.addEventListener('click', runCitySearch);
  input.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') runCitySearch();
  });
  clearBtn.addEventListener('click', clearCitySearch);
})();

// ─────────────────────────────────────────────────────────
// BACKGROUND MUSIC — completely self-contained, touches nothing else
// in this file. Only ever starts when the button itself is tapped —
// no auto-start on first touch/click anywhere in the app.
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// "WHAT'S THE STORY HERE?" — on-the-fly AI local history,
// spoken aloud, not displayed as text. Entirely self-contained:
// reads the same userLat/userLon that every other feature already
// uses (so it automatically respects an active city search), calls
// one new endpoint (api/story.js), and speaks the result with the
// browser's own built-in text-to-speech. Touches nothing else.
// ─────────────────────────────────────────────────────────
(function initStoryButton() {
  ...
  // Bumped every time the button is tapped fresh, or stopped. Any
  // in-flight fetch/audio callback checks this before acting, so a
  // stale response from a run that's since been stopped (or superseded
  // by a new tap) can't start talking after the fact.
  var requestId = 0;

  // iOS Safari only auto-allows playback on an <audio> element that has
  // actually been played once as a direct result of a user tap. It does
  // NOT re-check this per call — once an element has successfully played,
  // that same element keeps playing later even after a network delay.
  // The bug this fixes: previously every speak() call created a brand
  // new Audio object, so the SECOND phrase (the actual story, which only
  // starts after a network round trip + the first phrase finishing)
  // was a never-before-played element by the time .play() ran, and iOS
  // silently swallowed it — no error, just silence. Reusing one element
  // for the whole session avoids that entirely.
  ...
  function setState(state) {
    btn.classList.remove('asking', 'speaking');
  ...
  // Fetches real generated speech (Microsoft's neural voices, via the
  // api/speak.js endpoint) and resolves to a playable blob URL. Kept
  // separate from actually playing it, so the caller can start this
  // fetch early and play it whenever it's ready, instead of always
  // fetching and playing back-to-back.
  function fetchSpeechBlobUrl(text) {
    return fetch('https://roadguide-lime.vercel.app/api/speak?text=' + encodeURIComponent(text))
      .then(function(r) {
        if (!r.ok) throw new Error('Speech request failed (' + r.status + ')');
        return r.blob();
      })
      .then(function(blob) {
  ...
  // Plays an already-fetched blob URL on the single shared, pre-unlocked
  // <audio> element (see getSharedAudio above) and cleans it up once done.
  function playBlobUrl(url, myId, onDone) {
    if (myId !== requestId) {
    ...
    currentAudio = audio;
    // Clear any handlers from the previous phrase before reassigning
    // src, so a stale onended/onerror can't fire twice.
    audio.onended = null;
    audio.onerror = null;
    audio.src = url;
    function cleanup() {
  ...
  // Convenience wrapper: fetch then immediately play. Used for the short
  // intro phrase and one-off error messages, where there's nothing else
  // to overlap the fetch with.
  function speak(text, myId, onDone, isFallback) {
    fetchSpeechBlobUrl(text)
      .then(function(url) {
        if (myId !== requestId) { URL.revokeObjectURL(url); return; }
        playBlobUrl(url, myId, onDone);
      })
      .catch(function(err) {
        if (myId !== requestId) return;
        console.warn('Speech synthesis failed:', err.message);
        // Previously this failed in total silence — no sound, no
        // indication anything went wrong. Now it tries once to at
        // least say *something* short failed, rather than nothing at
        // all. isFallback stops this from ever looping if even the
        // short fallback phrase fails too.
        if (!isFallback) {
          speak("Something went wrong there.", myId, onDone, true);
        } else {
          onDone && onDone();
        }
      });
  ...
  window.tellMeTheStory = function() {
    if (busy) {
      // Tapped again while it's already talking — stop immediately
      // instead of being ignored or queuing another request.
      stopEverything();
      return;
    }
    var myId = ++requestId;
    if (userLat === null || userLon === null) {
      speak("I don't have a location to work with yet.", myId, function() { busy = false; });
      return;
    }
    ...
    // Start the location/AI text lookup immediately, in parallel with
    // the spoken intro below.
    var storyTextPromise = fetch('https://roadguide-lime.vercel.app/api/story?lat=' + encodeURIComponent(userLat)
      + '&lng=' + encodeURIComponent(userLon))
      .then(function(r) {
    ...
    // As soon as the story TEXT comes back, immediately start
    // synthesizing ITS speech too — do not wait for the intro phrase to
    // finish playing first. This overlaps the two slowest steps (the
    // location/AI lookup, and the story's own text-to-speech synthesis)
    // with the intro's fetch+playback time, instead of stacking every
    // step end-to-end. Previously this chain only started once the
    // intro finished, which is what caused the ~12 second dead-air gap.
    var storyAudioPromise = storyTextPromise
      .then(function(result) {
        var text;
        if (!result.ok) {
          var reason = (result.data && result.data.error) ? result.data.error : "an unknown server error";
          console.warn('Story endpoint returned an error:', reason);
          text = "There's a setup problem with the story feature: " + reason;
        } else {
          text = (result.data && result.data.story)
            ? result.data.story
            : "I couldn't find anything specific about this exact spot.";
        }
        return fetchSpeechBlobUrl(text);
      })
      .catch(function(err) {
        console.warn('Story lookup failed:', err.message);
        return fetchSpeechBlobUrl("Something went wrong reaching the story feature. Check your connection and try again.");
      });

    speak("What's the story here?", myId, function() {
      if (myId !== requestId) return; // stopped while the intro was playing
      setState('speaking');
      storyAudioPromise
        .then(function(url) {
          if (myId !== requestId) { URL.revokeObjectURL(url); return; }
          // Usually already fetched by now since it ran alongside the
          // intro — this just plays it. If it's genuinely still in
          // flight (a slow AI response), this waits for it here instead.
          playBlobUrl(url, myId, function() {
            busy = false;
            setState(null);
          });
        })
        .catch(function(err) {
          // Shouldn't normally happen — storyAudioPromise already has its
          // own .catch that falls back to an error phrase — but guards
          // against the fallback fetch itself failing too.
          if (myId !== requestId) return;
          console.warn('Story audio failed entirely:', err.message);
          busy = false;
          setState(null);
        });
    });
