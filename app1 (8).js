...
(function initStoryButton() {
  ...
  // api/speak.js endpoint) and plays it back like any other audio clip.
  function speak(text, myId, onDone, isFallback) {
    if (currentAudio) {
      currentAudio.pause();
      currentAudio = null;
    }
    fetch('https://roadguide-lime.vercel.app/api/speak?text=' + encodeURIComponent(text))
      .then(function(r) {
        if (!r.ok) throw new Error('Speech request failed (' + r.status + ')');
        return r.blob();
      })
      .then(function(blob) {
        if (myId !== requestId) return; // stopped or superseded while this was loading
        var url = URL.createObjectURL(blob);
        var audio = new Audio(url);
        currentAudio = audio;
        audio.onended = function() { URL.revokeObjectURL(url); onDone && onDone(); };
        audio.onerror = function() { URL.revokeObjectURL(url); onDone && onDone(); };
        audio.play().catch(function(err) {
  ...
  window.tellMeTheStory = function() {
    ...
    // Start the text lookup immediately, in parallel with the spoken
    // prompt below — this is two separate calls again (text, then
    // voice), not combined into one. Combining them into a single
    // request previously risked exceeding Vercel's hard 10-second
    // limit per request on this plan, since it forced the location
    // lookup, the AI call, AND voice generation all into one window.
    // Splitting them back apart means each piece gets its own fresh
    // 10-second budget, so this trades a little speed for actually
    // finishing reliably.
    ...
    speak("What's the story here?", myId, function() {
      if (myId !== requestId) return; // stopped while the prompt was playing

      storyTextPromise
        .then(function(result) {
          if (myId !== requestId) return; // stopped while the lookup was running
          setState('speaking');
          var text;
          if (!result.ok) {
        ...
        .catch(function(err) {
          ...
          setState('speaking');
          speak("Something went wrong reaching the story feature. Check your connection and try again.", myId, function() {
            if (myId !== requestId) return;
            busy = false;
            setState(null);
          });
