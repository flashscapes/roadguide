...
function restaurantCuisineBullet(r) {
  ...
  for (var i = 0; i < types.length; i++) {
    if (CUISINE_LABELS[types[i]]) return CUISINE_LABELS[types[i]] + ' restaurant';
  }
  return 'Restaurant';
}
...
function buildRestaurantSnapshot(r) {
  return [restaurantCuisineBullet(r), restaurantOutdoorBullet(r), restaurantPracticalBullet(r)];
}

function restaurantSnapshotHTML(r) {
  var bullets = buildRestaurantSnapshot(r);
  return '<ul class="rest-snapshot" style="margin:0 0 8px 0;padding-left:18px;font-size:12px;color:var(--dim);line-height:1.5;list-style:disc">'
...
function renderRestaurantCards() {
  ...
  for (var i = 0; i < HERO_COUNT; i++) {
    ...
    html += '<div class="card rank-' + (i + 1) + '" onclick="openRestaurant(' + i + ')">'
          ...
          +       '<div style="font-size:14px;margin-bottom:6px;color:var(--glo)">' + escHtml(rating) + '</div>'
          +       (price ? '<div style="font-family:\'DM Mono\',monospace;font-size:10px;color:var(--dim);margin-bottom:8px">' + escHtml(price) + '</div>' : '')
          +       '<a class="yt-link" href="' + escHtml(r.googleMapsUri || '#') + '" target="_blank" rel="noopener" onclick="event.stopPropagation()">📍 Open in Google Maps</a>'
          +     '</div>'
          +   '</div>'
          + '</div>';
  }
...
function renderRestaurantList() {
  var list = document.getElementById('list');
  if (!list) return;
  var items = restaurantResults.slice(HERO_COUNT, HERO_COUNT + 13);
  if (!items.length) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    html += '<div class="card card-mid" onclick="openRestaurant(' + idx + ')">'
          ...
          +     '<div class="card-fact">'
          +       restaurantSnapshotHTML(r)
          +       '<div style="font-size:12px;color:var(--glo)">' + escHtml(rating) + (r.priceLevel ? ' · ' + escHtml(restaurantPrice(r.priceLevel)) : '') + '</div>'
          +     '</div>'
          +   '</div>'
          + '</div>';
  }
