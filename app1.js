...
(function initStoryButton() {
  ...
  window.tellMeTheStory = function() {
    ...
    // Kick off the combined lookup + AI + voice request immediately, in
    // parallel with the spoken prompt below, instead of waiting for the
    // prompt to finish first. One combined request also replaces what
    // used to be two separate round trips (one for the story text, one
    // to turn it into audio) — cutting out an entire extra network trip
    // and a second cold start.
    var storyAudioPromise = fetch('https://roadguide-lime.vercel.app/api/tell-story?lat=' + encodeURIComponent(userLat)
    ...
    speak("What's the story here?", myId, function() {
      ...
      storyAudioPromise
        .then(function(r) {
          if (!r.ok) {
            return r.json().then(function(data) {
              throw new Error((data && data.error) ? data.error : 'Story request failed');
            });
          }
          return r.blob();
        })
        .then(function(blob) {
          ...
          var audio = new Audio(url);
          currentAudio = audio;
          audio.onended = function() { URL.revokeObjectURL(url); busy = false; setState(null); };
          audio.onerror = function() { URL.revokeObjectURL(url); busy = false; setState(null); };
          audio.play().catch(function(err) {
            console.warn('Audio playback failed:', err.message);
            URL.revokeObjectURL(url);
            busy = false;
        ...
        .catch(function(err) {
          ...
          speak("There's a problem getting the story: " + err.message, myId, function() {
            if (myId !== requestId) return;
            busy = false;
            setState(null);
          });
