...
function loadDopeDetours(destination) {
  ...
  fetch(dirUrl)
    ...
    .then(function(resultsPerWaypoint) {
      ...
      // Score with the SAME bucket-weight formula already used for
      // Explore/Cool Stuff (reused, not reinvented) — this is what
      // actually implements "prioritize fun/quirky/popular/
      // interesting over where they fall along the journey".
      var scored = merged.map(function(place) {
        ...
        var score = typeWeight + qualityBonus + reviewBonus + funBonus;
