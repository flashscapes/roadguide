...
function loadCoolStuff() {
  ...
  setMsg('Searching for nearby Hidden Gems…');
...
function renderCoolStuff() {
  ...
  if (!combined.length) {
    setMsg('No genuinely interesting Hidden Gems found within 15 miles right now.');
    if (countEl) countEl.textContent = 'No Hidden Gems found nearby';
    if (dc) dc.textContent = '0';
    if (cards) cards.innerHTML = '<div class="card"><div class="card-placeholder">No nearby Hidden Gems found</div></div>';
    if (list) list.innerHTML = '';
    return;
  }
