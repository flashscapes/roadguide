...
// Bullet 1: cuisine / what it's known for — derived only from Google's
// own "types" classification for this place.
...
// Bullet 2: outdoor seating — always one of exactly three states,
// straight from the provider's boolean field (or "unknown" if
// Google has no verified data for this place).
...
// Bullet 3: best available practical, visit-planning detail.
// Priority order: open/closed now → price level → rating/reviews →
// reservations → delivery/takeout → fallback if nothing is available.
function restaurantPracticalBullet(r) {
  if (r.openNow === true)  return 'Open now';
  if (r.openNow === false) return 'Closed now';

  var price = restaurantPrice(r.priceLevel);
  if (price) return 'Price level ' + price;
  ...
  if (r.reservable === true) return 'Reservations available';

  if (r.delivery === true || r.takeout === true) {
    if (r.delivery === true && r.takeout === true) return 'Delivery and takeout';
    if (r.delivery === true) return 'Delivery available';
...
function buildRestaurantSnapshot(r) {
  return [
    restaurantCuisineBullet(r),
    restaurantOutdoorBullet(r),
    restaurantPracticalBullet(r)
  ];
}

function restaurantSnapshotHTML(r) {
  var bullets = buildRestaurantSnapshot(r);

  return '<ul class="rest-snapshot" ' +
    'style="margin:0 0 8px 0;padding-left:18px;font-size:12px;' +
    'color:var(--dim);line-height:1.5;list-style:disc">' +
  ...
  for (var i = 0; i < 2; i++) {
    ...
    html +=
      '<div class="card rank-' + (i + 1) + '"' +
        ...
        '<div class="card-emoji" style="display:flex">' +
          '🍴' +
        '</div>' +
        ...
        '<div class="card-body">' +
          ...
          '<div class="card-fact">' +
            ...
            '<div style="font-size:14px;margin-bottom:6px;color:var(--glo)">' +
              escHtml(rating) +
            '</div>' +

            (price
              ? '<div style="font-family:\'DM Mono\',monospace;font-size:10px;color:var(--dim);margin-bottom:8px">' +
                  escHtml(price) +
                '</div>'
              : '') +
...
function renderRestaurantList() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    html +=
      '<div class="litem" onclick="openRestaurant(' + idx + ')">' +
        ...
        '<div class="litem-info">' +
          ...
          '<div class="litem-sub">' +
            escHtml(rating) +
            (r.priceLevel
              ? ' · ' + escHtml(restaurantPrice(r.priceLevel))
              : '') +
          '</div>' +
