...
// Looks up a real, verified photo for the currently-open overlay
// landmark and upgrades from the emoji once it arrives. Guards
// against the user tapping a different landmark before this
// resolves — a stale result should never overwrite a newer overlay.
function loadOverlayPhoto(lm, oImg, oEmoji) {
  if (!lm || lm.lat == null || lm.lon == null || !lm.name) return;

  fetch('https://roadguide-lime.vercel.app/api/landmark-photos', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ landmarks: [{ id: 0, name: lm.name, lat: lm.lat, lon: lm.lon }] })
  })
    ...
    .then(function(data) {
      if (overlayLandmark !== lm) return; // a different landmark opened meanwhile
      var photos = Array.isArray(data.photos) ? data.photos : [];
      var match = photos[0];
      if (!match || !match.photoRef) return;

      var photoUrl = 'https://roadguide-lime.vercel.app/api/photo?ref=' + encodeURIComponent(match.photoRef) + '&maxWidth=800';
      var testImg = new Image();
      testImg.onload = function() {
        if (overlayLandmark !== lm) return;
        oImg.src = photoUrl;
        oImg.style.display = 'block';
        oEmoji.style.display = 'none';
      };
      // onerror intentionally does nothing — emoji stays showing.
      testImg.src = photoUrl;
    })
    .catch(function(error) {
      console.error('Overlay photo lookup failed:', error);
    });
}
...
function openOverlay(idx) {
  ...
  // Static lm.photo is unreliable (unverified URLs, no runtime
  // fallback if broken) — show the emoji immediately, then upgrade
  // to a real, verified photo via loadOverlayPhoto() below.
  oImg.src             = '';
  oImg.style.display   = 'none';
  oEmoji.style.display = 'flex';
  oEmoji.textContent   = lm.emoji;
  loadOverlayPhoto(lm, oImg, oEmoji);
...
function resetTopBtn() {
  // The top Listen button was removed (superseded by Google Places
  // listings) — these elements no longer exist, so this safely
  // no-ops instead of throwing. openOverlay() calls this
  // unconditionally, so this guard matters for every landmark tap.
  var btn = document.getElementById('listenBtn');
  var lbl = document.getElementById('listenLabel');
  var sub = document.getElementById('listenSub');
  if (btn) btn.classList.remove('speaking');
  if (lbl) lbl.textContent = 'LISTEN';
  if (sub) sub.textContent = 'Tap to hear nearest landmark';
}
